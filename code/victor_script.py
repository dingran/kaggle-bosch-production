...
def Train32_33_34():
    ...
    response = pd.read_csv('../data/train_numeric.csv', usecols=['Id'])
...
def Test32_33_34():
    ...
    train_date_part = pd.read_csv('../data/test_date.csv', nrows=10000)
    date_cols = train_date_part.drop('Id', axis=1).count().reset_index().sort_values(by=0, ascending=False)
    date_cols['station'] = date_cols['index'].apply(lambda s: s.split('_')[1])
    date_cols = date_cols[date_cols['station'].isin(STATIONS)]
    date_cols = date_cols.drop_duplicates('station', keep='first')['index'].tolist()
    print(date_cols)
    train_date = pd.read_csv('../data/test_date.csv', usecols=['Id'] + date_cols)
    print(train_date.columns)
    train_date.columns = ['Id'] + STATIONS
    for station in STATIONS:
        train_date[station] = 1 * (train_date[station] >= 0)
    response = pd.read_csv('../data/test_numeric.csv', usecols=['Id'])
    response['Response_magic'] = 0
    ...
    train_start_date = pd.read_csv('../data/test_date.csv', usecols=['Id'] + date_cols)
    train_start_date['start_date'] = train_start_date[date_cols].min(axis=1)
    train_start_date = train_start_date.drop(date_cols, axis=1)
    print(train_start_date.shape)
    ...
    train_id = pd.read_csv('../data/test_numeric.csv', usecols=['Id'])
    print("train_id org")
    print(train_id)
    ...
    train_with_magic['Response_magic'] = 100 * train_with_magic['Magic'] + 10 * train_with_magic['S32'] + \
                                         train_with_magic['S33']
    train_with_magic['Response_magic'] = 1 * (
    (train_with_magic['Response_magic'] == 310) | (train_with_magic['Response_magic'] == 110))
    return train_with_magic
...
def GrabData():
    ...
    df_mindate.to_csv('debug_mindate.csv')
    ...
    blindtraindata.head(1000).to_csv('debug_victor.csv')
...
def Train():
    ...
    for i in range(folds):
        ...
        print('mcc:', best_mcc)
        print(matthews_corrcoef(train.Response.values,
                                y_pred))
